...
function renderStockCard(d) {
  const safeTicker = d.ticker.replace('.ST', '').replace(/[^a-zA-Z0-9]/g, '_');
  const id   = "stock-" + safeTicker;
  ...
  const rsVal = rs.outperforming === null
    ...
    : rs.outperforming
      ? `<span class="check">outperforming</span> <span class="muted">(${rs.stock_return > 0 ? '+' : ''}${rs.stock_return}% vs OMX ${rs.bench_return > 0 ? '+' : ''}${rs.bench_return}%)</span>`
      : `<span class="cross">underperforming</span> <span class="muted">(${rs.stock_return > 0 ? '+' : ''}${rs.stock_return}% vs OMX ${rs.bench_return > 0 ? '+' : ''}${rs.bench_return}%)</span>`;
  ...
  card.innerHTML = `
    <div class="card-header">
      <div>
        <div class="ticker-name">${d.name} <span style="font-size:0.9rem;color:#4f8ef7;font-weight:500">(${safeTicker})</span></div>
        <div class="ticker-price">${d.price} <span class="sek-tag">SEK</span> <span class="${d.daily_change >= 0 ? 'check' : 'cross'}">${d.daily_change >= 0 ? '+' : ''}${d.daily_change}%</span></div>
      </div>
      ${signalBadge(d)}
    </div>
    ...
    <div class="row" title="POC = highest volume price. VAH = distribution zone. VAL = accumulation zone."><span class="row-label">POC / VAH / VAL</span><span class="row-value">${inst.poc} / ${inst.vah} / ${inst.val} SEK</span></div>
...
async function loadStocks() {
  ...
  source.onmessage = function(e) {
    ...
    grid.innerHTML = "";
    results.forEach(r => {
      const safeTicker = r.ticker.replace('.ST', '').replace(/[^a-zA-Z0-9]/g, '_');
      const card = document.createElement("div");
      card.className = "card loading";
      card.id = "stock-" + safeTicker;
